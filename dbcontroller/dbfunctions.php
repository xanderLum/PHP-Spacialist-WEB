...
function addStaffByBusId($bus_id,$firstname,$lastname,$gender,$address,$staff_logo){
    $conn = conn();
    $sql = "INSERT INTO staff(bus_id,firstname,lastname,gender,address,staff_logo) VALUES((SELECT bus_id from business where bus_id=?),?,?,?,?,?)";
    //echo $sql;
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($bus_id,$firstname,$lastname,$gender,$address,$staff_logo));
    $staff_id=$conn->lastInsertId();
    console_log("addStaffByBusId staffId inserted=".$staff_id);
    return $staff_id;
    //$conn = null;
}
...
function updateStaffByBusId($firstname,$lastname,$gender,$address,$staff_logo,$staff_id){
    $conn = conn();
    $sql = "UPDATE staff SET firstname = ?, lastname = ?, gender = ?, address = ?, staff_logo=? WHERE staff_id = ? ";
    //echo $sql;
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($firstname,$lastname,$gender,$address,$staff_logo,$staff_id)); 
    $conn = null;
}
...
function getTransactionByBusId($bus_id){
    $conn = conn();
    $sql = "SELECT t.transaction_id,b.bus_name,u.firstname,t.payment_id,t.payment_transaction_id,t.payment_details,t.amount,t.created_dt from transaction t inner join business b on t.bus_id=b.bus_id inner join user u on t.user_id=u.user_id where t.bus_id=?";
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($bus_id));
    $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $conn = null;
